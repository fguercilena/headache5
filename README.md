# HEADACHE5 -> **H**i**E**r**A**rchical **DA**ta format 5, C++ **HE**ader library

A simple, header-only and single-header wrapper for the HDF5 C library for
C++20. It exposes some of the most commonly used HDF5 API features in a
structured, object-oriented way.

## HEADACHE5 is:
- a simple C++ wrapper for the most commonly used HDF5 entities (File, Graoup,
  DataSet, DataSpace, Attribute, common DataTypes), with clean API
- a simple C++ wrapper for the most common operations on those entities
  (create/open/close Files, Groups, Datasets, read/write data and attrbutes),
  with clean API

## HEADACHE5 is **NOT**:
- a higher level abstraction layer, it uses the same concepts as the HDF5 C API
- a link between fancy containers (STL, Eigen, etc...) and HDF5. You pass
  pointers to data to HEADACHE5 just like you would pass them to the C API.
  There are no plans to include such functionality
- something that exposes every HDF5 feature. HEADACHE5 knows nothing about
  compound datatypes, virtual datasets, parallel HDF5 or dimension scales.
  Support for those features and others might come in the future

### Installation:
The library is header only, so simply copy or simlink the *headache5.hpp* header
somewhere you like and *#include* it in your C++ project.

### Requirements:
- A compiler supporting C++23
- The C++ standard library
- The HDF5 library (to include and link against)

### Testing and examples:
The *test.cpp* file can be compiled and run by simply typing `make` (requires
*gcc*). Besides testing the main features of the library, you can take a look
at it as an usage example.
